...
def lead_left(rx, ry):
    return ry >= EGO_HALF_WIDTH and EGO_HALF_LENGTH <= rx 

def lead_0(rx, ry):
    return -EGO_HALF_WIDTH < ry < EGO_HALF_WIDTH and EGO_HALF_LENGTH <= rx 
...
def follow_0(rx, ry):
    return -EGO_HALF_WIDTH < ry < EGO_HALF_WIDTH and rx < -EGO_HALF_LENGTH

def follow_right(rx, ry):
    return ry <= -EGO_HALF_WIDTH and  rx < -EGO_HALF_LENGTH
...
def generate_output_text(ego_coords, npc_coords, ego_vel, npc_vel, timestamps, npc_ids):
    ...
    for i in range(1, len(processed_data)):
        ...
        if (prev_pos == "lead-right" and curr_pos in ["lead0"]) or \
            ...
            (prev_pos == "left" and curr_pos in ["lead0"]):
    ...
    for i in range(1, len(processed_data)):
        prev_pos = processed_data[i-1]['relative_pos']
        curr_pos = processed_data[i]['relative_pos']
        if (prev_pos == "lead0" and curr_pos in ["lead-right", "lead-left"]):
    ...
    detection_results = {
        ...
        # ★追加: 状態数と圧縮率の統計データ
        'total_frames': original_count,
        'num_abstract_states': compressed_count,
    ...
    output_lines.append("")
    output_lines.append("加速シナリオ検出 (Faster & Follow):")
    ...
    output_lines.append("")
    output_lines.append("減速シナリオ検出 (Slower & Lead):")
...
def process_folder(input_folder, output_folder):
    ...
    for i, (success, error, detection_results) in enumerate(results):
        ...
        if success and detection_results:
            ...
            # ★追加: CSVレコードの作成 (検出結果に状態数・圧縮率が含まれているため展開するだけでOK)
            csv_record = {
                'scenario': input_path.name,
                'filename': original_file_name,
                **detection_results 
            }
            folder_csv_records.append(csv_record)
    ...
    for key, label in [
        ...
        if detection_stats[key]['count'] > 0 and detection_stats[key]['count'] <= 10:
            ...
            for filename in sorted(detection_stats[key]['files'])[:5]:
                print(f"  • {filename}")
                add_summary(f"  • {filename}")
            print(f"  ... (他 {detection_stats[key]['count'] - 5}件)")
...
if __name__ == "__main__":
    ...
    for scenario_name in TARGET_SCENARIOS:
        ...
        output_folder = BASE_OUTPUT_DIR / "9area" / scenario_name
    ...
    if all_scenario_data:
        ...
        # 出力先: D:/datamapping/9area/summary_all.csv
        csv_output_path = BASE_OUTPUT_DIR / "9area" / "summary_all.csv"
        csv_output_path.parent.mkdir(parents=True, exist_ok=True)
        
        try:
            df = pd.DataFrame(all_scenario_data)
            # カラムの順序を整える（読みやすくするため）
