...
- 一方、Egoを同期遷移（strans）で実際にCPDのcarとして追加したワールド座標系アニメーション（11.5節のv2、`with_ego_track`ベース）は、この規模（33箱、`max_step=6`）では同期遷移グループの組み合わせ数（NPCの通常遷移数×Egoの`max_step`）が膨れ上がり、480秒経っても計算が終わらなかった。そのため大規模デモでは、Egoを同期させない単純な静止Ego版（11.3節の`gif_viz.py`、`render_scenarios_gif`）にフォールバックしたところ97.19秒で描画できた（`multilog_large.gif`）。**Egoを同期させたワールド座標系アニメーションのスケーラビリティは未解決の課題として残る**（stransの生成方法を`ntrans`ごとではなく、必要な同期グループだけに絞るなどの最適化が今後考えられる）。
