...
この経緯自体が「近い/遠いの境界と遠方セルサイズは、ログを区別できるように設定しなければならない」という当然の懸念を裏付けている。そこで、`build_union_model_near_far_grid`にも`build_union_model`と同じ安全装置を追加した。具体的には`find_distinguishing_near_far_grid`（`find_distinguishing_grid`の非一様格子版）を追加し、`rx_near_range`・`rx_near_cell`は呼び出し側の指定のまま固定し、`rx_far_cell`だけを（デフォルトで、または`auto_grid=True`のとき）全ログが区別できるまで自動的に細かくしていく。`rx_far_cell`が`rx_near_cell`まで縮まれば、実質的に一様格子（`build_union_model`に`rx_near_cell`をそのまま`gx`として渡した場合）と同じ区別能力に収束するため、その一様格子で区別できるログ集合であれば必ず有限回で見つかる。さらに、`auto_grid=False`で明示的な値を指定した場合も含め、最終的に得られた格子で全ログが区別できているかを関数内部で必ず検証し、区別できていなければ（一様格子版の`build_union_model`と同様に）`ValueError`を送出するようにした。実際に`rx_near_range=25m, rx_far_cell=10m`（区別できない組み合わせ）を`auto_grid=False`で指定すると`ValueError`になり、`auto_grid=True`（デフォルト）で同じ値から始めると自動的に区別できる格子（この場合は`rx_far_cell`が`rx_near_cell`の5mまで縮まり、一様格子と同じ33箱に収束）を見つけて成功することを確認した。つまり「区別できない非一様格子が黙って使われてしまう」ことは起きない。

なお、`rx_near_range`自体（近い/遠いの境界をどこに置くか）は今回のところ手動で決めており、これを自動で探索する仕組みはまだない（`rx_far_cell`の自動細分化だけでは、境界の置き方自体が悪い場合、`rx_near_cell`まで縮んで一様格子に戻るだけでモデル縮小の効果が得られないことがある。近い/遠いの境界も含めて「区別を保ったまま最大限まとめる」自動探索は今後の課題）。
