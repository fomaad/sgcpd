...
11.3節のGIFは、モデルが持つ`(lane, position)`をそのまま「ego基準の相対座標」の格子とみなし、egoを`lane=0, position=0`に静止させて描いていた。しかし実際にはegoも道路上を前進しており、positionが変化しないのは「NPCとの相対距離」を状態として使っているからに過ぎない。論文的な見た目にする際、egoも動いているように見せたいという要望を受け、`logverify/world_frame_gif.py`を追加した。

- Ego自身が一定速度で前進すると仮定し、`ego_world_position(step) = step × ego_speed`（`ego_speed`は格子セル/ステップの単位で指定するパラメータ、デフォルト1）とする。
- NPCの絶対位置は `ego_world_position(step) + モデルが持つ相対position` として計算する。lane（横方向）はもともとego基準の相対値（隣接レーン=-1/1など）なので変換不要で、そのまま「egoからの左右オフセット」として使う。
- カメラはワールド座標系に固定（egoを追従しない）。そのため、egoもNPCも画面内を前進方向へ移動していく様子が同じアニメーションの中で見える。
...
- 相対distanceが順序尺度（距離帯）である方法Bと、実際の格子（メートル単位）である方法Cとで、`ego_speed`の意味合いが厳密には異なる（前者は「1ステップで前進する順序尺度上の目盛り数」、後者は「1ステップで前進する格子セル数」であり、どちらも実際の車速を表すものではない）点には注意が必要。egoの絶対速度自体はログの相対座標だけからは復元できない情報であり、あくまで見た目のためのパラメータである。
